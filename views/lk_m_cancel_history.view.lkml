view: lk_m_cancel_history {
    derived_table: { sql:
      ...
      CASE
        ...
        when cancel_reason = 43 then 'Non-receipt of documents (U30)'
        when cancel_reason = 44 then 'Non-receipt of documents'
        when cancel_reason = 45 then 'Non-receipt of payment'
        when cancel_reason = 46 then 'Panel decline'
       ...
       CASE
         ...
         WHEN cancel_reason = 21 THEN 'Cheaper quote obtained'
         WHEN cancel_reason in(4,5,46) THEN 'MTA decline'
         WHEN cancel_reason in(11,12,17) THEN 'No longer driving'
         WHEN cancel_reason is null THEN 'No payment made (CFI)'
         WHEN cancel_reason in(22,29,31,32,33,34,40,43,44,45) THEN 'Non-receipt of payment/documents'
         WHEN cancel_reason = 26 THEN 'NTU'
       ...
       CASE
         ...
         WHEN cancel_reason = 14 THEN 'Cancelled by firm (careless misrepresentation)'
         ...
         WHEN cancel_reason in(3,6,7,16,20,23,35,40,41,42, 18,31,32,33,34,43,44) THEN 'Other'
...
FROM (SELECT c.*,
      ...
      FROM (SELECT a.*,
            ...
            on prem.tia_reference = a.tia_reference and a.annual_cover_start_dttm = prem.annual_cover_start_dttm*/
              LEFT JOIN lk_m_cancel_history b
